...
public class MainActivity extends AppCompatActivity {
    ...
    private int[] solarFireTime = {60, 70, 85};
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        output.setText(coreTime(delayMinute, volumeData, solarFireTime, coreFrequency) + " секунд");
    }
    ...
    private int volumeTime(int coreFrequency, int volumeData, int solarFlareTime) {
        return ((volumeData * solarFlareTime) / coreFrequency) / 60;
    }
    ...
    private int coreTime(int[] delayMinute, int[] volumeData, int[] solarFireTime, int frequency) {
        ...
        for (int i = 0; i < volume.length; i++) { // инициализация данного массива
            volume[i] = volumeTime(frequency, volumeData[i], solarFireTime[i]); // определение времени обработки компьютером данных от одного спутника
        }
